...
const {determineContentType, prepareAuditData, mapAudits} = require("./helper");
const {validateCreateItem, validateUpdateItem} = require("./validators");
const {ACTIONS, STATUS, ERROR_MESSAGES} = require("./constants");
const {generateHome, generatePrint, generateList, generate404} = require("./renderers");
...
module.exports.create = async (req, res) => {
  ...
  await db.createAudit(prepareAuditData(ACTIONS.CREATE, {...item, id: newId, listId: id}));
...
module.exports.update = async (req, res) => {
  ...
  await db.createAudit(prepareAuditData(action, {id, listId, body, updatedData}));
  res.writeHead(STATUS.OK, {"Content-Type": "application/json"});
  return res.end();
};
...
module.exports.remove = async (req, res) => {
  ...
  await db.createAudit(prepareAuditData(ACTIONS.DELETE, {id, listId, ...deletedData.value}));
  res.writeHead(STATUS.OK);
  return res.end();
};
