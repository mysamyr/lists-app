...
const generateMeta = ({title, scripts = [], stylesheets = [], lang = "ua"}) => ([
  `<html lang=${lang}>
    <head>
      <meta charset="UTF-8">
...
const categoryMapper = ({name, id}) =>
  `<li data-id="${id}">${name}</li>`;
const listMapper = (data, schema) =>
  `<li data-id="${data.id}">${getView(schema, data)}<div><span id="edit">~</span><span id="delete">&times;</span></div></li>`;
...
module.exports.generateList = (list) => {
  ...
  return [start, `<div class="row">`, header, backBtn, `</div>`, "<ul>", content, "</ul>", btns, addBtn, dialog, end].join("");
};
...
module.exports.generatePrint = (list) => {
  ...
  return [start, `<div class="row">`, header, backBtn, `</div>`, content, end].join("");
};
