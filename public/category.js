window.addEventListener("DOMContentLoaded", async function () {
  ...
  function validateCreationInputs(inputs, fields) {
    ...
    for (let {field, description, type, min, max} of fields) {
      ...
      if (type === "string" && (typeof value !== "string" || value.length < min || value.length > max)) {
        return {error: `Поле ${description} має бути рядком і включати від ${min} до ${max} символів`};
      }
      if (type === "number") {
        value = value === "" ? 0 : +value;
        if ((value < min || value > max)) {
          return {error: `Поле ${description} має бути числом від ${min} до ${max}`};
        }
      }
      data[field] = value;
    }
  ...
  function renderDetails({item, fields}) {
    const lines = fields.map(({field, description, postfix}) => `<p>${description}: <b id="${field}">${item[field]}${postfix || ""}</b></p>`).join("\n");
    return `
    ${lines}
    <div class="btns">
  ...
  function renderCreationForm(fields) {
    const inputs = fields.map(({field, description, type}) => `
        <input name="${field}" type="${type === "string" ? "text" : "number"}" placeholder="${description}">
    `).join("\n");
    return `
    <form>
      ${inputs}
  ...
  async function openDelete(id, name) {
    ...
    const [saveBtn, closeBtn] = document.querySelectorAll("dialog .btn");
    saveBtn.addEventListener("click", async function () {
      await fetch(`/list/${listId}/item/${id}`, {method: "DELETE"}).then(async function (data) {
        if (!data.ok) {
          const res = await data.json();
          throw new Error(res.error);
        }
