...
import { createDialog, createDiv, createParagraph } from "../utils/elements";
import {
	getListData,
	getListItems,
	getParentData,
...
export default async (id) => {
	...
	if (isHomePage || !listDetails) {
		...
		if (isHomePage) {
			const configsLink = createParagraph();
			configsLink.innerText = "Go to Configs";
			configsLink.addEventListener("click", () => {
				return navigate(URLS.CONFIGS);
			});
			body.appendChild(configsLink);
		}
		renderDialog(body);
	} else {
		body.innerText = "";
