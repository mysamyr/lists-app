...
import { createDiv, createListItem, createSpan } from "../utils/elements";
import DeleteModal from "./modals/DeleteModal";
import { deleteRequest, putRequest } from "../api";
import RenameModal from "./modals/RenameModal";
import { validateName } from "../utils/validators";

const onClickListItem = async (e, id, data) => {
	e.preventDefault();
	pushBreadcrumbs(data);
	const path = URLS.GET_LIST_DETAILS_$(id);
...
const getButtons = (id, name) => {
	...
	renameBtn.addEventListener("click", (e) => onRenameListItem(e, id, name));
	deleteBtn.addEventListener("click", (e) => onDeleteListItem(e, id));
...
export default (data, view) => {
	...
	listItem.addEventListener("click", (e) => onClickListItem(e, id, data), {
		once: true,
	});
